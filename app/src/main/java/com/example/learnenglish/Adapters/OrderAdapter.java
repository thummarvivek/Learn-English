...
public class OrderAdapter extends RecyclerView.Adapter<OrderAdapter.Myclass> {
    ...
    @NonNull
    @Override
    public OrderAdapter.Myclass onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v= LayoutInflater.from(parent.getContext()).inflate(R.layout.ordercustom,parent,false);
        return new Myclass(v);
    }

    @Override
    public void onBindViewHolder(@NonNull OrderAdapter.Myclass holder, int position) {
        ...
        holder.obname.setText(arrayList.get(position).getName());
        holder.oedition.setText(arrayList.get(position).getBookEdition());
        holder.oprice.setText(arrayList.get(position).getBookPrice());
